...
class CircuitBreakerManager {
    ...
    getBreaker(upstreamUrl, options = {}) {
        ...
        // Breaker wraps async functions
        const breakerFunction = async (requestFn) => requestFn();
    ...
    async execute(upstreamUrl, fn, options = {}) {
        ...
        try {
            ...
            if (error.name === 'CircuitOpenError') {
                logger.error('Circuit breaker is open', { upstream: upstreamUrl, error: error.message });
    ...
    recordFailure(upstreamUrl, error = {}) {
        ...
        // - 5xx server errors
        const networkErrors = ['ECONNRESET', 'ETIMEDOUT', 'ECONNREFUSED', 'ENOTFOUND', 'ECONNABORTED'];
