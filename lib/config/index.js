...
const config = Object.freeze({
    ...
    timeouts: Object.freeze({
        ...
        healthCheck: parseIntEnv('HEALTH_CHECK_TIMEOUT_MS', defaults.timeouts.healthCheck, { min: 100 }),
    ...
    retry: Object.freeze({
        ...
        initialDelay: parseIntEnv('RETRY_INITIAL_DELAY_MS', defaults.retry.initialDelay, { min: 10 }),
    ...
    circuitBreaker: Object.freeze({
        timeout: parseIntEnv('CIRCUIT_BREAKER_TIMEOUT_MS', defaults.circuitBreaker.timeout, { min: 1000 }),
        errorThresholdPercentage: parseIntEnv('CIRCUIT_BREAKER_ERROR_THRESHOLD', defaults.circuitBreaker.errorThresholdPercentage, { min: 1, max: 100 }),
        resetTimeout: parseIntEnv('CIRCUIT_BREAKER_RESET_TIMEOUT_MS', defaults.circuitBreaker.resetTimeout, { min: 1000 }),
    ...
    healthCheck: Object.freeze({
        intervalMs: parseIntEnv('HEALTH_CHECK_INTERVAL_MS', defaults.healthCheck.intervalMs, { min: 1000 }),
        unhealthyThreshold: parseIntEnv('HEALTH_CHECK_UNHEALTHY_THRESHOLD', defaults.healthCheck.unhealthyThreshold, { min: 1 }),
        healthyThreshold: parseIntEnv('HEALTH_CHECK_HEALTHY_THRESHOLD', defaults.healthCheck.healthyThreshold, { min: 1 })
    }),

    // Connection pooling
    connectionPool: Object.freeze({
        maxSockets: parseIntEnv('MAX_SOCKETS', defaults.connectionPool.maxSockets, { min: 1, max: 1024 }),
        maxFreeSockets: parseIntEnv('MAX_FREE_SOCKETS', defaults.connectionPool.maxFreeSockets, { min: 1, max: 1024 }),
    ...
    // Validation errors (for startup checks)
    validationErrors
});
 ...
 */
config.getValidationErrors = () => [...validationErrors];
